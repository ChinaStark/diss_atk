...
def sql_sfx(sql: Any) -> str:
    """Normalize SQL for intra-group dedup: collapse whitespaces + strip + lowercase."""
    if sql is None:
        return ""
...
def dedup_group_items(group_items: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Deduplicate by pred_sql_sfx within one group."""
    seen: set[str] = set()
    deduped: list[dict[str, Any]] = []
    for item in group_items:
        key = item["pred_sql_sfx"]
        if key in seen:
            continue
        seen.add(key)
        deduped.append(item)
...
def build_item(item: dict[str, Any]) -> dict[str, Any]:
    label = int(item.get("correctness", 0))
    item_type = "pos" if label == 1 else "neg"
    return {
        "exec_idx": item.get("idx"),
        "sql": item.get("pred_sql", ""),
        "ground_truth": item["ground_truth"],
        "label": label,
        "type": item_type,
        "db_file": item.get("db_file", ""),
...
def process_items(items: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    4) create pred_sql_sfx and deduplicate per group
    5) build single-item records (only pos/neg, one SQL per item)
    """
    ...
    for _, group_items in grouped.items():
        ...
        for item in deduped:
            ...
            if correctness in (0, 1):
                output_items.append(build_item(item))

    return output_items


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build NL2SQL single-item pos/neg data from raw list JSON.")
...
def main() -> None:
    ...
    samples = process_items(data)

    output_path.parent.mkdir(parents=True, exist_ok=True)
    with output_path.open("w", encoding="utf-8") as f:
        json.dump(samples, f, ensure_ascii=False, indent=args.indent)

    pos_count = sum(1 for x in samples if x.get("type") == "pos")
    neg_count = sum(1 for x in samples if x.get("type") == "neg")
    print(f"done. total_items={len(samples)} pos={pos_count} neg={neg_count}")
